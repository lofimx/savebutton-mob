...
Every day, almost all people jot down little things for themselves. These may be a lightweight bookmark of a website, or small text blurbs. Maybe an image or PDF. Such bookmarks and blurbs are inherently immutable, living on a timeline that is rarely (if ever) revisited.
...
The core functionality of Kaya comes from retrieval: looking up old bookmarks, blurbs, and files. Arbitrary files can be added, but images and PDFs are the most likely formats other than `.url` (bookmarks) and `.md` (blurbs).
